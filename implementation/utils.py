"""
Script: contains utility methods needed to process training data
@author: nelsoonc
...
def load_data(csv_path, dataset_path, command_flag, test_size):
    columns = ['filename', 'throttle', 'steering', 'command']
    data = pd.read_csv(csv_path, names=columns, header=0)
    ...
    if command_flag == 'left':
        image_filename = data.filename[data.command=='left'].values
        y = data.steering[data.command=='left'].values
    elif command_flag == 'straight':
        image_filename = data.filename[data.command == 'straight'].values
        y = data.steering[data.command=='straight'].values
    elif command_flag == 'right':
        image_filename = data.filename[data.command == 'right'].values
        y = data.steering[data.command=='right'].values
    x = []
    # drop_indexes = []
    ...
    print('[INFO] Total images imported:', np.shape(x)[0])
